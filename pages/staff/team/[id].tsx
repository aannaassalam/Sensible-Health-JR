...
export default function EditTeam() {
  ...
  const data = useQueries({
    queries: [
      { queryKey: ["user_list"], queryFn: getStaffList },
      {
        queryKey: ["team", id],
    ...
    combine: (result) => ({
      staffs: result[0].data,
      existing_team: result[1].data,
  ...
  useEffect(() => {
    if (!data.isLoading && data) {
      setStaff_list({
        selected_staffs: data.existing_team.employees,
        all_staffs: data.staffs.filter(
          (_staff: IStaff) =>
            !data.existing_team.employees.some(
              (_existing_staff: IStaff) => _existing_staff.id === _staff.id
            )
        )
      });
      methods.setValue("teamName", data.existing_team.teamName);
    }
  }, [data.isLoading]);
  ...
  return (
    <DashboardLayout isLoading={data.isLoading}>
      <StyledPage>
        <Stack
          direction="row"
          alignItems="center"
            ...
            <Grid container spacing={4}>
              ...
              >
                ...
                <DragOverlay>
                  {activeId ? (
                    <Draggable
                      item={data.staffs.find(
                        (_data: IStaff) => _data.id === activeId
                      )}
                      container=""
                    />
                  ) : null}
